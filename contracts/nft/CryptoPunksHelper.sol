...
contract CryptoPunksHelper is NFTEscrow, Ownable {
    constructor(address punksAddress) NFTEscrow(punksAddress) {}
    ...
    function _encodeFlashEscrowPayload(uint256 _idx) internal view override returns (bytes memory) {
        return abi.encodeWithSignature("transferPunk(address,uint256)", address(this), _idx);
    }
}
