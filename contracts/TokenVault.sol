...
contract TokenVault is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "TokenVault: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "TokenVault: No tokens");
        if (share == 0) {
        ...
        require(
            from != address(this) || token_ == USE_ETHEREUM || amount.add(total.elastic) <= _tokenBalanceOf(token),
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "TokenVault: cannot empty");
        totals[token] = total;
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "TokenVault: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "TokenVault: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "TokenVault: Wrong amount");
